...
export async function runDripEmails(): Promise<void> {
  ...
  for (const patient of patients) {
    ...
    // GDPR: optionally require communication consent.
    if (dripConfig.requireCommunicationConsent) {
      const consent = await consentService.getConsentStatus(
        patient._id.toString(),
        "communication_consent"
      );
      if (!consent.hasAcceptedLatest) {
        skipped++;
        continue;
      }
    }

    const result = await reserveAndSend(patient, nextTemplate);
    if (result === "sent") sent++;
  ...
  console.log(
    `[drip] run finished — ${patients.length} active subscribers scanned, ${sent} sent, ${skipped} skipped, ${failed} failed`
  );
}
